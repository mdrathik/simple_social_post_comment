...
$allpost = $obj->allpost();
$most_value=$_GET['most_value'];
if($most_value==1){
    $most= $obj->most();
}
else if($most_value==2){
    $most =$obj->mostc();
}
else{
...
if(isset($_POST['likes'])) {
    ...
    echo "<script>location='most.php?most_value=1'</script>";
}
    ...
    <div class="row">
        <div class="col-md-12 up_button_div">
            <form action="" method="post">
                <a type="submit" name="most_like" class="btn btn-danger btn-xsm btn-circle text-uppercase btn1x" href="most.php?most_value=1" id="reply"><span class="glyphicon glyphicon-heart"></span>Most Like</a>
                <a class="btn btn-warning btn-xsm btn-circle text-uppercase btn2x" data-toggle="collapse" href="most.php?most_value=2"><span class="glyphicon glyphicon-comment"></span> &nbsp;Most comments</a>
                <a class="btn btn-success btn-xsm btn-circle text-uppercase btn3x" data-toggle="collapse" href="index.php">&nbsp; All Post</a>
            </form>
        </div>
        ...
        <div class="main_body col-md-10">
            <div id="scroll_div">
             <?php
                foreach ($most as $key1) { ?>

                    <?php
                    $mal=$key1['status_id'];
                    $mostlike= $obj->mostlike($key1);
                    foreach ($mostlike as $key) {
                        $comment_count=$key['id'];
                            ...
                            <form method="post" action="">
                                <div class="cols-sm-10">
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" name="name" id="name"  placeholder="Anonymous Nickname"/>
                                    </div>
                                </div>
                                    <textarea id="field" name="status" onkeyup="countChar(this)" placeholder="Heard, seen or experience kindness? Share your feel good story with the rest of Singapore.
" ></textarea>
                                <ul>
